...
class SegmentedEditQuoteGrid extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  renderActionItems(cellInfo) {
    ...
    const reconfigure = cellInfo.original.canReconfigure ? <a title={this.context.intl.formatMessage({ ...messages.recongifure })} className={cellInfo.original.isDisableReconfiguration ? 'disabled-link' : 'link'} onClick={() => { browserHistory.push(`/reconfigureproducts?ids=${cellInfo.original.id}/${this.props.quoteData.id}/${this.props.quoteData.priceBookId}`); }}><Glyphicon glyph="wrench" /></a> : <span className="blank"></span>;
